#!/usr/bin/env python3
"""
在综述产出目录追加「综述修订记录.md」一条：含记录时间（本地 + UTC）、用户说明摘要、交付文件名、合并/纠正摘要摘录。
"""
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="综述产出目录（与综述 .md 同级或为其父目录，须已存在）",
    )
    parser.add_argument(
        "--kind",
    ...
    parser.add_argument(
        "--artifacts",
        default="",
        help="本轮交付文件名，多个用英文逗号分隔，如：topic_20260408143025.md,topic_20260408143025.docx",
    )
    parser.add_argument(
        "--log-name",
        default=DEFAULT_LOG,
