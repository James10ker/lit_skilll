...
论文综述中的分类框架、研究路线、时间线和方法谱系可使用 fenced mermaid；避免用 ASCII 文字箭头图替代正式图示。
...
用法：
  python tools/mermaid_render.py -i draft.md -o review.md
  # 默认在同目录生成 review.docx；失败时 stderr 会给出可复制的 md_to_docx 命令
  python tools/mermaid_render.py -i draft.md -o out/review.md --docx out/custom.docx
  python tools/mermaid_render.py -i draft.md -o review.md --no-docx   # 仅 Markdown
