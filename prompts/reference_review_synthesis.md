...
1. 按参考综述来源和检索词收集候选文献；不可访问来源用公开 API 近似，并显式记录差异。
2. 进入 `structured_literature_pipeline.md`：用 DOI、arXiv ID、规范化标题及作者—年份去重，并合并预印本/会议版/期刊版。
3. 按主题相关性、对象、方法透明度、来源质量、经典性、时效性和多样性筛选文献，不以引用量单独决定纳入。
4. 建立统一 Paper Store；国家/地区、机构等缺失字段保持缺失，不得推断补齐。
...
7. 建立 Paper Card、Theme Synthesis 和 Claim–Evidence Store v2，确保 RQ1-RQ5 的重要结论均有读取权限匹配、定位明确的证据。
8. 证据权限验证通过后，按参考综述的问题结构合成完整 LaTeX 工程；写后执行 citation verification，再运行 `latex_delivery.md` 的质量门禁。
