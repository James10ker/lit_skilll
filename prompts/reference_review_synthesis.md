...
6. 生成流程图、年度发文量柱状图和双五年 topic 词云；只有真实国家、机构或共著字段充分时才用 `tools/render_collaboration_networks.py` 生成合作连接图。参考 Figure 7/8 时优先生成国家/地区与机构两张力导向网络，保持“节点面积=子集发文量、线宽=合作强度、颜色=社区”的语义；不得用异构 bibliographic evidence map 替代。词云使用宽幅高对比双面板并完整披露两个窗口的记录数与计数口径。每张图必须来自本次数据、全部使用英文标签并通过 report。若 topic-year 数据充分，再生成年度主题比例图回答 RQ5。
